...
class mod_gnrquiz_attempt_walkthrough_testcase extends advanced_testcase {
    ...
    public function test_gnrquiz_attempt_walkthrough() {
        ...
        $timenow = time();
        $attempt = gnrquiz_create_attempt($quizobj, 1, false, $timenow, false, $user1->id);
        ...
        // Check quiz grades.
        $grades = gnrquiz_get_user_grades($quiz, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'gnrquiz', $quiz->id, $user1->id);
    ...
    public function test_gnrquiz_with_random_question_attempt_walkthrough() {
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            $timenow = time();
            $attempt = gnrquiz_create_attempt($quizobj, 1, false, $timenow);

            gnrquiz_start_new_attempt($quizobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    public function test_gnrquiz_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        ...
        if ($this->quizwithvariants === null) {
            ...
            gnrquiz_add_gnrquiz_question($calc->id, $this->quizwithvariants, 0);
        }
        ...
        // Select variant.
        gnrquiz_start_new_attempt($quizobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check quiz grades.
        $grades = gnrquiz_get_user_grades($this->quizwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'gnrquiz', $this->quizwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
