...
class mod_gnrquiz_class_testcase extends basic_testcase {
    public function test_cannot_review_message() {
        ...
        $this->assertEquals('',
            $quizobj->cannot_review_message(mod_gnrquiz_display_options::DURING));
        $this->assertEquals('',
            $quizobj->cannot_review_message(mod_gnrquiz_display_options::IMMEDIATELY_AFTER));
        $this->assertEquals(get_string('noreview', 'gnrquiz'),
            $quizobj->cannot_review_message(mod_gnrquiz_display_options::LATER_WHILE_OPEN));
        $this->assertEquals(get_string('noreview', 'gnrquiz'),
            $quizobj->cannot_review_message(mod_gnrquiz_display_options::AFTER_CLOSE));
