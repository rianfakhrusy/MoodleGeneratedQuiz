...
class behat_mod_quiz extends behat_question_base {
     ...
     */
    public function gnrquiz_contains_the_following_questions($quizname, TableNode $data) {
        global $DB;

        $quiz = $DB->get_record('gnrquiz', array('name' => $quizname), '*', MUST_EXIST);
        ...
        foreach ($data->getHash() as $questiondata) {
            ...
            // Add the question.
            gnrquiz_add_gnrquiz_question($questionid, $quiz, $page, $maxmark);
     ...
     */
    public function gnrquiz_contains_the_following_sections($quizname, TableNode $data) {
        global $DB;
     ...
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the "(?P<gnrquiz_name_string>(?:[^"]|\\")*)" quiz with:$/
     * @param string $questiontype
     * @param string $quizname
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_gnrquiz_with($questiontype, $quizname, TableNode $questiondata) {
        $quizname = $this->escape($quizname);
        $editquiz = $this->escape(get_string('editquiz', 'gnrquiz'));
        $quizadmin = $this->escape(get_string('pluginadministration', 'gnrquiz'));
