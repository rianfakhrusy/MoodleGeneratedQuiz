@mod @mod_gnrquiz
Feature: Edit gnrquizzes where some questions require the previous one to have been completed
  In order to create gnrquizzes where later questions can only be seen after earlier ones are answered
  As a teacher
  I need to be able to configure this on the Edit gnrquiz page
  ...
  @javascript
  Scenario: Question dependency cannot apply to deferred feedback gnrquizzes so UI is hidden
    Given the following "activities" exist:
      | activity   | name   | intro              | course | idnumber | preferredbehaviour |
  ...
  @javascript
  Scenario: Question dependency cannot apply to gnrquizzes with sequential navigation so UI is hidden
    Given the following "activities" exist:
  ...
  Scenario: When questions are reordered, the dependency icons are updated correctly
    ...
    And I follow "Quiz 1"
    And I follow "Edit gnrquiz"
    When I move "Question 1" to "After Question 3" in the gnrquiz by clicking the move icon
    Then "Question 2 cannot be attempted until the previous question 1 has been completed • Click to change" "link" should be visible
    And "No restriction on when question 3 can be attempted • Click to change" "link" should be visible
