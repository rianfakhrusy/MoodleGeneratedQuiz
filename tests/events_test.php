...
class mod_gnrquiz_events_testcase extends advanced_testcase {
    ...
    protected function prepare_gnrquiz_data($ispreview = false) {
        ...
        gnrquiz_add_gnrquiz_question($saq->id, $quiz);
        gnrquiz_add_gnrquiz_question($numq->id, $quiz);
        ...
        $timenow = time();
        $attempt = gnrquiz_create_attempt($quizobj, 1, false, $timenow, $ispreview);
    ...
    public function test_attempt_submitted() {
        ...
        list($quizobj, $quba, $attempt) = $this->prepare_gnrquiz_data();
    ...
    public function test_attempt_started() {
        ...
        $attempt = gnrquiz_create_attempt($quizobj, 1, false, time(), false, 2);
        ...
        $expected = array($quizobj->get_courseid(), 'gnrquiz', 'attempt', 'review.php?attempt=' . $attempt->id,
    ...
    public function test_edit_page_viewed() {
        ...
        $expected = array($course->id, 'gnrquiz', 'editquestions', 'view.php?id=' . $quiz->cmid, $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_deleted() {
        ...
        // Trigger and capture the event.
        $sink = $this->redirectEvents();
        gnrquiz_delete_attempt($attempt, $quizobj->get_quiz());
        $events = $sink->get_events();
        $event = reset($events);
        ...
        $this->assertEquals(context_module::instance($quizobj->get_cmid()), $event->get_context());
        $expected = array($quizobj->get_courseid(), 'gnrquiz', 'delete attempt', 'report.php?id=' . $quizobj->get_cmid(),
    ...
    public function test_preview_attempt_deleted() {
        ...
        list($quizobj, $quba, $previewattempt) = $this->prepare_gnrquiz_data(true);

        // Delete a preview attempt, capturing events.
        $sink = $this->redirectEvents();
        gnrquiz_delete_attempt($previewattempt, $quizobj->get_quiz());
    ...
    public function test_report_viewed() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'gnrquiz', 'report', 'report.php?id=' . $quiz->cmid . '&mode=overview',
            $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_reviewed() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'gnrquiz', 'review', 'review.php?attempt=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_summary_viewed() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'gnrquiz', 'view summary', 'summary.php?attempt=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_override_updated() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'gnrquiz', 'edit override', 'overrideedit.php?id=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_override_deleted() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'gnrquiz', 'delete override', 'overrides.php?cmid=' . $quiz->cmid, $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_viewed() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'gnrquiz', 'continue attempt', 'review.php?attempt=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_preview_started() {
        ...
        $this->assertEquals(context_module::instance($quizobj->get_cmid()), $event->get_context());
        $expected = array($quizobj->get_courseid(), 'gnrquiz', 'preview', 'view.php?id=' . $quizobj->get_cmid(),
            $quizobj->get_quizid(), $quizobj->get_cmid());
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_question_manually_graded() {
        ...
        $this->assertEquals(context_module::instance($quizobj->get_cmid()), $event->get_context());
        $expected = array($quizobj->get_courseid(), 'gnrquiz', 'manualgrade', 'comment.php?attempt=2&slot=3',
            $quizobj->get_quizid(), $quizobj->get_cmid());
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
