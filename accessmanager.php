...
class gnrquiz_access_manager {
    ...
    protected function make_rules($quizobj, $timenow, $canignoretimelimits) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['gnrquizaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('gnrquizaccess', '', 'rule.php');
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'gnrquiz'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_gnrquiz_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_gnrquiz_preflight_check_form'])) {
            $method = 'get';
        }
     ...
     *
     * @param object $attempt the data from the relevant gnrquiz_attempts row.
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(gnrquiz_attempt::IN_PROGRESS, gnrquiz_attempt::OVERDUE))) {
