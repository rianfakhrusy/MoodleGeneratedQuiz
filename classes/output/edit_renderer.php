...
class edit_renderer extends \plugin_renderer_base {
    ...
    public function gnrquiz_information(structure $structure) {
        ...
        $output = html_writer::span(
                    get_string('numquestionsx', 'gnrquiz', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'gnrquizopeningstatus',
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_gnrquiz_grade()));
        $output .= html_writer::empty_tag('input', array('type' => 'submit',
                'name' => 'savechanges', 'value' => get_string('save', 'gnrquiz')));
    ...
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {
        ...
        $header = html_writer::tag('span', get_string('repaginatecommand', 'gnrquiz'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'gnrquiz'),
        );
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'gnrquiz');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'gnrquiz', $select) .
    ...
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'gnrquiz', $section->heading),
    ...
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'gnrquiz');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'gnrquiz') . ' ' . $help,
    ...
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'gnrquiz', $section->heading);
        $url = new \moodle_url('/mod/gnrquiz/edit.php',
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'gnrquiz');
        }
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'gnrquiz', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'gnrquiz');
        }
        ...
        $returnurl = new \moodle_url('/mod/gnrquiz/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
        $params = array('returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion');
        $url = new \moodle_url('/mod/gnrquiz/addrandom.php', $params);
        ...
        if ($page) {
            $title = get_string('addrandomquestiontopage', 'gnrquiz', $page);
        } else {
            $title = get_string('addrandomquestionatend', 'gnrquiz');
        }
    ...
    public function question_preview_icon($gnrquiz, $question, $label = null, $variant = null) {
        ...
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'gnrquiz');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'gnrquiz');
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'gnrquiz');
            $image = $this->pix_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'gnrquiz');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'gnrquiz', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'gnrquiz'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'gnrquiz', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'gnrquiz'),
                    'moodle', array('title' => ''));
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = gnrquiz_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'gnrquiz').' '.$title));
    ...
    public function random_question(structure $structure, $slot, $pageurl) {
        ...
        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = gnrquiz_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'gnrquiz');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
                'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'gnrquiz'), array('class' => 'mod_gnrquiz_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'gnrquiz')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'gnrquiz'),
                )
            )
        );
    ...
    protected function random_question_form(\moodle_url $thispageurl, \question_edit_contexts $contexts, array $pagevars) {
        ...
        $randomform = new \gnrquiz_add_random_form(new \moodle_url('/mod/gnrquiz/addrandom.php'),
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/gnrquiz/edit_rest.php';
        $config->sectionurl = '/mod/gnrquiz/edit_rest.php';
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'gnrquiz', '');
