 ...
 */
class moodle1_mod_gnrquiz_handler extends moodle1_mod_handler {
    ...
    public function process_gnrquiz($data) {
        ...
        if (is_null($data['sumgrades'])) {
            $data['sumgrades'] = 0;
            // TODO for user data: gnrquiz_attempts SET sumgrades=0 WHERE sumgrades IS NULL.
            // TODO for user data: gnrquiz_grades.grade should be not be null , convert to default 0.
        }
        ...
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid,
                ...
                'contextid' => $contextid));
        $this->xmlwriter->begin_tag('gnrquiz', array('id' => $instanceid));
    ...
    public function on_gnrquiz_question_instances_start() {
        $this->xmlwriter->begin_tag('question_instances');
    }

    public function on_gnrquiz_question_instances_end() {
        $this->xmlwriter->end_tag('question_instances');
    }

    public function process_gnrquiz_question_instance($data) {
        $this->write_xml('question_instance', $data, array('/question_instance/id'));
    }

    public function on_gnrquiz_feedbacks_start() {
        $this->xmlwriter->begin_tag('feedbacks');
    }

    public function on_gnrquiz_feedbacks_end() {
        $this->xmlwriter->end_tag('feedbacks');
    }

    public function process_gnrquiz_feedback($data) {
    ...
    /**
     * This is executed when we reach the closing </MOD> tag of our 'gnrquiz' path
     */
    public function on_gnrquiz_end() {
        ...
        // Write inforef.xml.
        $this->open_xml_writer("activities/gnrquiz_{$this->moduleid}/inforef.xml");
