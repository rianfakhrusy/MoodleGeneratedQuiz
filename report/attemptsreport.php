 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class gnrquiz_attempts_report extends gnrquiz_default_report {
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'gnrquiz');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'gnrquiz');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'gnrquiz');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'gnrquiz');
    }
     ...
     * @param mod_gnrquiz_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_gnrquiz_attempts_report_options $options, $collapsible) {
