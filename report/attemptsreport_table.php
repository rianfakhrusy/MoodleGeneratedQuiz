 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class gnrquiz_attempts_report_table extends table_sql {
    ...
    /** @var object mod_gnrquiz_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/gnrquiz/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'gnrquiz'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return gnrquiz_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/gnrquiz/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'gnrquiz')));
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join($this->sql->from, 'gnrquiza.uniqueid',
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['gnrquiza.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a href="javascript:select_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectall', 'gnrquiz') . '</a> / ';
        echo '<a href="javascript:deselect_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectnone', 'gnrquiz') . '</a> ';
        echo '&nbsp;&nbsp;';
        $this->submit_buttons();
        echo '</div>';
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/gnrquiz:deleteattempts', $this->context)) {
            echo '<input type="submit" id="deleteattemptsbutton" name="delete" value="' .
                    get_string('deleteselected', 'gnrquiz_overview') . '"/>';
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'gnrquiz')));
        }
    }
}
