...
require_once($CFG->dirroot . '/mod/gnrquiz/report/attemptsreport_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gnrquiz_overview_table extends gnrquiz_attempts_report_table {
    ...
    protected function add_average_row($label, $users) {
        ...
        if ($this->options->slotmarks) {
            ...
            $qubaids = new qubaid_join($from, 'gnrquiza.uniqueid', $where, $params);
    ...
    protected function submit_buttons() {
        if (has_capability('mod/gnrquiz:regrade', $this->context)) {
            echo '<input type="submit" name="regrade" value="' .
                    get_string('regradeselected', 'gnrquiz_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        return html_writer::link(new moodle_url('/mod/gnrquiz/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'gnrquiz')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/gnrquiz/report/overview/report.php to see what fields are present,
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'gnrquiz_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'gnrquiz_overview');
        }
    }
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/gnrquiz:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('gnrquiz_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return gnrquiz_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
