 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gnrquiz_responses_settings_form extends mod_gnrquiz_attempts_report_form {

    protected function other_preference_fields(MoodleQuickForm $mform) {
        $mform->addGroup(array(
            $mform->createElement('advcheckbox', 'qtext', '',
                get_string('questiontext', 'gnrquiz_responses')),
            $mform->createElement('advcheckbox', 'resp', '',
                get_string('response', 'gnrquiz_responses')),
            $mform->createElement('advcheckbox', 'right', '',
                get_string('rightanswer', 'gnrquiz_responses')),
        ), 'coloptions', get_string('showthe', 'gnrquiz_responses'), array(' '), false);
        $mform->disabledIf('qtext', 'attempts', 'eq', gnrquiz_attempts_report::ENROLLED_WITHOUT);
        $mform->disabledIf('resp',  'attempts', 'eq', gnrquiz_attempts_report::ENROLLED_WITHOUT);
        $mform->disabledIf('right', 'attempts', 'eq', gnrquiz_attempts_report::ENROLLED_WITHOUT);
    }
    ...
    public function validation($data, $files) {
        ...
        if ($data['attempts'] != gnrquiz_attempts_report::ENROLLED_WITHOUT && !(
            ...
            $errors['coloptions'] = get_string('reportmustselectstate', 'gnrquiz');
        }
    ...
    protected function other_attempt_fields(MoodleQuickForm $mform) {
        ...
        if (gnrquiz_allows_multiple_tries($this->_customdata['gnrquiz'])) {
