...
class gnrquiz_statistics_table extends flexible_table {
    ...
    public function statistics_setup($gnrquiz, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'gnrquiz_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'gnrquiz_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'gnrquiz');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'gnrquiz_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'gnrquiz_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'gnrquiz_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'gnrquiz_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'gnrquiz_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'gnrquiz_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'gnrquiz_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'gnrquiz_statistics');
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'gnrquiz_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'gnrquiz_statistics')));
                ...
                if ($questionstat->get_variants() || $questionstat->get_sub_question_ids()) {
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('slotstructureanalysis', 'gnrquiz_statistics', $number)));
                } else {
                    // Question cannot be broken down into sub-questions or variants. Link will show response analysis page.
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'gnrquiz_statistics')));
                }
            }
        }
        ...
        if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'gnrquiz_statistics') . '<br />' . $name;
        }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($questionstat->negcovar) {
            $negcovar = get_string('negcovar', 'gnrquiz_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'gnrquiz_statistics'),
