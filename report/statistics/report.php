...
require_once($CFG->dirroot . '/mod/gnrquiz/report/statistics/statistics_form.php');
require_once($CFG->dirroot . '/mod/gnrquiz/report/statistics/statistics_table.php');
...
class gnrquiz_statistics_report extends gnrquiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        $reporturl = new moodle_url('/mod/gnrquiz/report.php', $pageoptions);

        $mform = new gnrquiz_statistics_settings_form($reporturl, compact('gnrquiz'));
        ...
        $this->table = new gnrquiz_statistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'gnrquiz_statistics');
        } else {
            $report = get_string('questionstatsfilename', 'gnrquiz_statistics');
        }
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && !$groupstudents) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'gnrquiz_statistics'));
                }
            }
        ...
        if ($everything) { // Implies is downloading.
            ...
            if ($variantno === null &&
                ...
                if (!$this->table->is_downloading()) {
                    ...
                    echo $OUTPUT->heading(get_string('slotstructureanalysis', 'gnrquiz_statistics', $number), 3);
                }
    ...
    protected function output_individual_question_data($quiz, $questionstat) {
        ...
        if ($questionstat->variant !== null) {
            $questioninfotable->data[] = array(get_string('variant', 'gnrquiz_statistics'), $questionstat->variant);

        }
        $questioninfotable->data[] = array(get_string('questiontype', 'gnrquiz_statistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($questionstat->question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'gnrquiz_statistics'),
        ...
        $labels = array(
            's' => get_string('attempts', 'gnrquiz_statistics'),
            'facility' => get_string('facility', 'gnrquiz_statistics'),
            'sd' => get_string('standarddeviationq', 'gnrquiz_statistics'),
            'random_guess_score' => get_string('random_guess_score', 'gnrquiz_statistics'),
            'intended_weight' => get_string('intended_weight', 'gnrquiz_statistics'),
            'effective_weight' => get_string('effective_weight', 'gnrquiz_statistics'),
            'discrimination_index' => get_string('discrimination_index', 'gnrquiz_statistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'gnrquiz_statistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'gnrquiz_statistics'), 3);
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($questionstat->question);
        echo $OUTPUT->heading(get_string('questionstatistics', 'gnrquiz_statistics'), 3);
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_question_preview_urls($question->questiontext, $question->id,
                ...
                $this->context->id, 'gnrquiz_statistics');
    ...
    protected function output_individual_question_response_analysis($question, $variantno, $s, $reporturl, $qubaids,
        ...
        $qtable = new gnrquiz_statistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'gnrquiz_statistics'), 3);
            ...
            if (!empty($question->number) && !is_null($variantno)) {
                $questiontabletitle = get_string('analysisnovariant', 'gnrquiz_statistics', $a);
            } else if (!empty($question->number)) {
                $questiontabletitle = get_string('analysisno', 'gnrquiz_statistics', $a);
            } else if (!is_null($variantno)) {
                $questiontabletitle = get_string('analysisvariant', 'gnrquiz_statistics', $a);
            } else {
                $questiontabletitle = get_string('analysisnameonly', 'gnrquiz_statistics', $a);
            }

            if ($this->table->is_downloading() == 'xhtml') {
                $questiontabletitle = get_string('analysisofresponsesfor', 'gnrquiz_statistics', $questiontabletitle);
            }
    ...
    protected function download_gnrquiz_info_table($quizinfo) {
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('gnrquizinformation', 'gnrquiz_statistics'));
    ...
    protected function get_progress_trace_instance() {
        if ($this->progress === null) {
            if (!$this->table->is_downloading()) {
                $this->progress = new \core\progress\display_if_slow(get_string('calculatingallstats', 'gnrquiz_statistics'));
    ...
    protected function everything_download_options(moodle_url $reporturl) {
        global $OUTPUT;
        return $OUTPUT->download_dataformat_selector(get_string('downloadeverything', 'gnrquiz_statistics'),
    ...
    protected function output_caching_info($lastcachetime, $quizid, $groupstudents, $whichattempts, $reporturl) {
        ...
        $output .= get_string('lastcalculated', 'gnrquiz_statistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'gnrquiz_statistics'));
    ...
    protected function clear_cached_data($qubaids) {
        global $DB;
        $DB->delete_records('gnrquiz_statistics', array('hashcode' => $qubaids->get_hash_code()));
