...
require_once($CFG->dirroot . '/mod/gnrquiz/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gnrquiz_grading_report extends gnrquiz_default_report {
    ...
    public function display($gnrquiz, $cm, $course) {
        ...
        require_capability('mod/gnrquiz:grade', $this->context);
        $shownames = has_capability('gnrquiz/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('gnrquiz/grading:viewidnumber', $this->context);
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'gnrquiz_grading'));
            }
        }
    ...
    protected function base_url() {
        return new moodle_url('/mod/gnrquiz/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'gnrquiz_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'gnrquiz_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'gnrquiz_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'gnrquiz_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'gnrquiz_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'gnrquiz_grading');
        $table->head[] = get_string('questionname', 'gnrquiz_grading');
        $table->head[] = get_string('tograde', 'gnrquiz_grading');
        $table->head[] = get_string('alreadygraded', 'gnrquiz_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'gnrquiz_grading');
        }
        $table->head[] = get_string('total', 'gnrquiz_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new gnrquiz_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'gnrquiz_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'gnrquiz_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'gnrquiz_grading', $a), 3);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'value' => get_string('saveandnext', 'gnrquiz_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'gnrquiz_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'gnrquiz_grading', $a);
