...
class quiz {
    ...
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/gnrquiz:preview', $this->context);
        }
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'gnrquiz', $this->view_url());
            }
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new gnrquiz_access_manager($this, $timenow,
                    has_capability('mod/gnrquiz:ignoretimelimits', $this->context, null, false));
        }
    ...
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/gnrquiz/view.php?id=' . $this->cm->id;
    }
    ...
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/gnrquiz/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/gnrquiz/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
        return $url;
    ...
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/gnrquiz/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/gnrquiz/review.php', array('attempt' => $attemptid));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/gnrquiz/summary.php', array('attempt' => $attemptid));
    }
     ...
     *
     * @param int $when One of the mod_gnrquiz_display_options::DURING,
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_gnrquiz_exception($this, 'questionnotloaded', $id);
        }
    }
...
class gnrquiz_attempt {
    ...
    /** @var array of gnrquiz_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_gnrquiz_display_options cache for the appropriate review options. */
     ...
     * @param array $conditions passed to $DB->get_record('gnrquiz_attempts', $conditions).
     */
    ...
    /**
     * Static function to create a new gnrquiz_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new gnrquiz_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return gnrquiz_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'gnrquiz');
                }
    ...
    /** @return object the row from the gnrquiz_attempts table. */
    ...
    /** @return string one of the gnrquiz_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/gnrquiz:viewreports')) {
            return false;
        }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_gnrquiz_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/gnrquiz:attempt';
        } else {
            $capability = 'mod/gnrquiz:reviewmyattempts';
        }
        ...
        if ($this->has_capability('mod/gnrquiz:viewreports') ||
                $this->has_capability('mod/gnrquiz:preview')) {
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'gnrquiz'));
        } else {
            return '';
        }
    }
     ...
     */
    public function render_question($slot, $reviewing, mod_gnrquiz_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_gnrquiz_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'gnrquiz', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'gnrquiz');
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_gnrquiz_renderer $renderer, $thispageurl = '') {
    ...
    public function get_navigation_panel(mod_gnrquiz_renderer $output,
        ...
        $bc->attributes['id'] = 'mod_gnrquiz_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_gnrquiz_navblock_title';
        $bc->title = html_writer::span(get_string('gnrquiznavigation', 'gnrquiz'), '', array('id' => 'mod_gnrquiz_navblock_title'));
     ...
     *
     * The {@link mod_gnrquiz_links_to_other_attempts} object returned contains an
    ...
    public function links_to_other_attempts(moodle_url $url) {
        ...
        $links = new mod_gnrquiz_links_to_other_attempts();
     ...
     * @return mod_gnrquiz_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('gnrquiz_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        gnrquiz_send_overdue_message($this);
    }
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/gnrquiz/' . $script . '.php' . $fragment,
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('gnrquiz_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
 ...
 * @since      Moodle 2.9
 */
class gnrquiz_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class gnrquiz_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class gnrquiz_nav_panel_base {
    /** @var gnrquiz_attempt */
    ...
    public function __construct(gnrquiz_attempt $attemptobj,
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new gnrquiz_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new gnrquiz_nav_question_button();
            $button->id          = 'gnrquiznavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'gnrquiz');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'gnrquiz');
        } else {
            return get_string('viewed', 'gnrquiz');
        }
    }

    public function render_before_button_bits(mod_gnrquiz_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_gnrquiz_renderer $output);
 ...
 * @since      Moodle 2.0
 */
class gnrquiz_attempt_nav_panel extends gnrquiz_nav_panel_base {
    ...
    public function render_before_button_bits(mod_gnrquiz_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'gnrquiz'),
                array('id' => 'gnrquiznojswarning'));
    }

    public function render_end_bits(mod_gnrquiz_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'gnrquiz'), array('class' => 'endtestlink')) .
 ...
 * @since      Moodle 2.0
 */
class gnrquiz_review_nav_panel extends gnrquiz_nav_panel_base {
    public function get_question_url($slot) {
