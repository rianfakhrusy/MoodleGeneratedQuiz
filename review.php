...
$url = new moodle_url('/mod/gnrquiz/review.php', array('attempt'=>$attemptid));
if ($page !== 0) {
    $url->param('page', $page);
} else if ($showall) {
...
if ($attemptobj->is_own_preview()) {
    $strreviewtitle = get_string('reviewofpreview', 'gnrquiz');
...
$PAGE->set_title($attemptobj->get_gnrquiz_name());
...
if ($attempt->state == gnrquiz_attempt::FINISHED) {
    ...
    $timetaken = get_string('unfinished', 'gnrquiz');
}
...
if ($attemptobj->has_capability('mod/gnrquiz:viewreports')) {
    ...
    if ($attemptlist) {
        $summarydata['attemptlist'] = array(
            'title'   => get_string('attempts', 'gnrquiz'),
            'content' => $attemptlist,
        );
    }
}
...
$summarydata['startedon'] = array(
    'title'   => get_string('startedon', 'gnrquiz'),
    'content' => userdate($attempt->timestart),
);

$summarydata['state'] = array(
    'title'   => get_string('attemptstate', 'gnrquiz'),
    'content' => gnrquiz_attempt::state_name($attempt->state),
);

if ($attempt->state == gnrquiz_attempt::FINISHED) {
    $summarydata['completedon'] = array(
        'title'   => get_string('completedon', 'gnrquiz'),
        'content' => userdate($attempt->timefinish),
    );
    $summarydata['timetaken'] = array(
        'title'   => get_string('timetaken', 'gnrquiz'),
        'content' => $timetaken,
    );
}
...
if (!empty($overtime)) {
    $summarydata['overdue'] = array(
        'title'   => get_string('overdue', 'gnrquiz'),
        'content' => $overtime,
    );
}
