 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_gnrquiz_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param gnrquiz_attempt $attemptobj an instance of gnrquiz_attempt.
     ...
     * @param mod_gnrquiz_display_options $displayoptions instance of mod_gnrquiz_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(gnrquiz_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_gnrquiz_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(gnrquiz_attempt $attemptobj, $slot, $seq,
            mod_gnrquiz_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(gnrquiz_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
     ...
     *
     * @param gnrquiz_attempt $attemptobj instance of gnrquiz_attempt
     ...
     * @param mod_gnrquiz_display_options $displayoptions instance of mod_gnrquiz_display_options
     */
    public function questions(gnrquiz_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_gnrquiz_display_options $displayoptions) {
        $output = '';
     ...
     * @param gnrquiz_attempt $attemptobj instance of gnrquiz_attempt
     */
    public function finish_review_link(gnrquiz_attempt $attemptobj) {
     ...
     */
    public function review_next_navigation(gnrquiz_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
    ...
    public function countdown_timer(gnrquiz_attempt $attemptobj, $timenow) {
        ...
        return html_writer::tag('div', get_string('timeleft', 'gnrquiz') . ' ' .
                html_writer::tag('span', '', array('id' => 'gnrquiz-time-left')),
                array('id' => 'gnrquiz-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'gnrquiz'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param gnrquiz_nav_panel_base $panel instance of gnrquiz_nav_panel_base
     */
    public function navigation_panel(gnrquiz_nav_panel_base $panel) {
     ...
     * @return string HTML fragment.
     */
    protected function render_gnrquiz_nav_question_button(gnrquiz_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod);
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'gnrquiz');
        }
     ...
     *
     * @param mod_gnrquiz_links_to_other_attempts $links
     */
    protected function render_mod_gnrquiz_links_to_other_attempts(
            mod_gnrquiz_links_to_other_attempts $links) {
        $attemptlinks = array();
     ...
     * @param gnrquiz_attempt $attemptobj Instance of gnrquiz_attempt
     * @param int $page Current page number
     * @param gnrquiz_access_manager $accessmanager Instance of gnrquiz_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->gnrquiz_notices($messages);
     ...
     */
    public function gnrquiz_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'gnrquiz'), 3) .
                $this->access_messages($messages), 'gnrquizaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param gnrquiz_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'gnrquiz'));
            $delay = 5;
        } else {
     ...
     *
     * @param gnrquiz_attempt $attemptobj
     * @param mod_gnrquiz_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_gnrquiz_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'gnrquiz'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'gnrquizsummaryheading';
            }
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'gnrquizsummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == gnrquiz_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'gnrquiz'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'gnrquiz'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == gnrquiz_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'gnrquiz'), null,
                    get_string('submitallandfinish', 'gnrquiz')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == gnrquiz_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'gnrquiz', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'gnrquiz', userdate($duedate));
        }
     ...
     * @param mod_gnrquiz_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_gnrquiz_view_object $viewobj) {
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'gnrquiz'), 'get',
     ...
     * @param mod_gnrquiz_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_gnrquiz_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_gnrquiz_preflight_check_form from ' .
                    'gnrquiz_access_manager::get_preflight_check_form, not a warning message string.');
        }
    ...
    public function view_information($gnrquiz, $cm, $context, $messages) {
        ...
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'gnrquizinfo');
        }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'gnrquiz'), 3);
    }
    ...
    public function view_table($gnrquiz, $context, $viewobj) {
        ...
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'gnrquiz');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'gnrquiz');
        $table->align[] = 'left';
        $table->size[] = '';
        ...
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'gnrquiz');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'gnrquiz');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'gnrquiz');
                } else {
     ...
     * @param gnrquiz_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case gnrquiz_attempt::OVERDUE:
                return get_string('stateoverdue', 'gnrquiz') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'gnrquiz',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case gnrquiz_attempt::FINISHED:
                return get_string('statefinished', 'gnrquiz') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'gnrquiz',
    ...
    public function view_result_info($gnrquiz, $context, $cm, $viewobj) {
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'gnrquiz'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'gnrquizteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'gnrquiz'));
            $button->add_action(new popup_action('click', $url, 'gnrquizpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'gnrquiz'),
                    array('title' => get_string('reviewthisattempt', 'gnrquiz')));
        }
    }
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'gnrquiz'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'gnrquiz'), FORMAT_MARKDOWN, $options);
...
class mod_gnrquiz_links_to_other_attempts implements renderable {
    /**
...
class mod_gnrquiz_view_object {
    ...
    public $attemptobjs;
    /** @var gnrquiz_access_manager $accessmanager contains various access rules. */
