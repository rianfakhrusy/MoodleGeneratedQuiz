YUI.add('moodle-mod_gnrquiz-util-slot', function (Y, NAME) {
...
Y.Moodle.mod_gnrquiz.util.slot = {
    ...
    reorderPageBreaks: function() {
        ...
        slots.each(function(slot, key) {
            ...
            pagebreaklink.set('title', M.util.get_string(action, 'gnrquiz'));
            pagebreaklink.setData('action', action);
            // Update the image title.
            var icon = pagebreaklink.one(this.SELECTORS.ICON);
            icon.set('title', M.util.get_string(action, 'gnrquiz'));
            icon.set('alt', M.util.get_string(action, 'gnrquiz'));
    ...
    updateDependencyIcon: function(slot, requiresprevious) {
        ...
        if (requiresprevious) {
            link.set('title', M.util.get_string('questiondependencyremove', 'gnrquiz', a));
            link.setData('action', 'removedependency');
            icon.set('alt', M.util.get_string('questiondependsonprevious', 'gnrquiz'));
            icon.set('src', M.util.image_url('t/locked', 'moodle'));
        } else {
            link.set('title', M.util.get_string('questiondependencyadd', 'gnrquiz', a));
            link.setData('action', 'adddependency');
            icon.set('alt', M.util.get_string('questiondependencyfree', 'gnrquiz'));
