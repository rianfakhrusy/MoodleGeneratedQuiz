YUI.add('moodle-mod_gnrquiz-toolboxes', function (Y, NAME) {
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'gnrquiz', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * @module mod_gnrquiz-resource-toolbox
 * @namespace M.mod_gnrquiz.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'gnrquiz', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_gnrquiz.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_gnrquiz.util.slot.remove(element);
    ...
    edit_maxmark : function(ev, button, activity) {
        ...
        var instancemaxmark  = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class'   : 'resource',
                'field'   : 'getmaxmark',
                'id'      : Y.Moodle.mod_gnrquiz.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_gnrquiz.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_gnrquiz.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_gnrquiz.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_gnrquiz.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_gnrquiz.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_gnrquiz.util.slot.reorderSlots();
        Y.Moodle.mod_gnrquiz.util.slot.reorderPageBreaks();
        Y.Moodle.mod_gnrquiz.util.page.reorderPages();
        Y.Moodle.mod_gnrquiz.util.slot.updateOneSlotSections();
        Y.Moodle.mod_gnrquiz.util.slot.updateAllDependencyIcons();
    },

    NAME : 'mod_gnrquiz-resource-toolbox',
    ATTRS : {
        courseid : {
            'value' : 0
        },
...
M.mod_gnrquiz.resource_toolbox = null;
M.mod_gnrquiz.init_resource_toolbox = function(config) {
    M.mod_gnrquiz.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_gnrquiz.resource_toolbox;
};
/* global TOOLBOX, BODY, SELECTOR */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'gnrquiz', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'gnrquiz', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'gnrquiz', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'gnrquiz', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'gnrquiz', response.instancesection));
                    }
                }
            });
        }
...
M.mod_gnrquiz.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_gnrquiz-gnrquizbase",
        "moodle-mod_gnrquiz-util-slot",
